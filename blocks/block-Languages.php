...
if ($useflags == 1) {
	...
	if(!empty($menulist[$i])) {
	    ...
	    $altlang = ucfirst($tl);
	    $content .= "<a href=\"index.php?newlang=$tl&amp;page=".base64_encode(CurrentURL())."\"><img src=\"images/language/flag-$tl.png\" border=\"0\" alt=\"$altlang\" title=\"$altlang\" hspace=\"3\" vspace=\"3\"></a> ";
	}
    }
    $content .= "</div>";
} else {
    ...
    $content .= "</select>
    <input type='hidden' name='page' value='".base64_encode(CurrentURL())."' />
    </form></div>";
}
