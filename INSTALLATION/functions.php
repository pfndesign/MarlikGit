	...
	function create_first($name, $url, $email, $pwd, $user_new)
	{
		...
		if (empty($first)) {
		 ...
		 ('$name', '$the_adm', '$url', '$email', '$pwd', '0', '1', '')");
			if (!$rez) {
				die("<div class='error'>" . TEXT_ERROR_DB . "<br><br>" . _GOBACK . "</div>");
			}
			$date = date("Y-m-d");
			$currentDomain = getenv("HTTP_HOST");
			$currenturl = !empty($currentDomain) ? $currentDomain : SPONSER_URL;
			file_put_contents(BASE_PATH . ".setting", str_replace(
				["startdate='" . $sitesettings['startdate'], "nukeurl='" . $sitesettings['nukeurl']],
				["tracking='" . $date, "nukeurl='" . $currenturl],
				file_get_contents(BASE_PATH . ".setting")
			));
			if ($user_new == 1) {
...
function USV_INSTALL_HEADER()
{
	...
	echo "<body>\n";
?>
	<div class="head_1"><img src="<?php echo INCLUDES_ACP ?>style/images/admin_logo.png" border="0"></div>
	<div class="head_2" style="margin-bottom: 20px;"><a href="./install.php"><img src="images/icon/house.png"></a>
