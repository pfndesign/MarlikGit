...
class Tags
{
	...
	{ //in this function we pass tags through to be inserted or if they already exist increase their count
		global $db, $prefix;
		$count = count($tags);
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if (!$tid and $tags[$i] != "") {
				...
				if ($i != ($count - 1)) $tag_sql .= ',';
				else $tag_sql .= ';';
			} else {
				$db->sql_query('UPDATE `' . $prefix . '_tags` SET `count` = `count` + 1 WHERE `tid` = "' . $tid . '" LIMIT 1');
				if ($i == ($count - 1)) {
	...
	function update_tag($tid, $new_tag)
	{ //it does what it says
		global $db, $prefix;
		$tid = (int) $tid;
		$new_tag = $new_tag;
	...
	{ // tags should be passed in this format: tag1,tag2,tag3,...
		global $db, $prefix;
		$tags = explode(",", $tags);
	...
	function change_story_tags($tags, $sid)
	{ // tags should be passed in this format: tag1,tag2,tag3,...
		global $db, $prefix;
	...
	function get_tag_by_ids($tids)
	{ //tag ids should be passed in this format to this function: ' id1 id2 id3 ... '
		global $db, $prefix;
		$tids = explode(" ", $tids);
		$c = count($tids);
		$wclse = '';
	...
	function input_tags($name, $ex_tags = '')
	{ // this one creates the input area for entering tags
		global $db, $prefix;
...
?>
		<script type="text/javascript" charset="utf-8">
			$(function() {
				...
				$.ajax({
					url: '<?php echo ADMIN_OP ?>Tags&act=query',
					dataType: 'json',
					success: function(r) {
...
";
	...
	function countdown($tags)
	{ // and this is where we decrease tags' count by one
		global $db, $prefix;
