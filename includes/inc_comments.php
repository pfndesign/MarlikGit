...
?>
	...
	?>
		...
		<article class="comment <?php echo $oe; ?>" id="comment-<?php echo $row[0] ?>">
			<header class="head">
				<figure class="avatar">
					<img src="<?php echo avatar_me($row[5]) ?>" width="60" height="60" alt="<?php echo $row[5] ?>" />
				</figure>
				...
				<div class="rating" id="comments"><span class="rate-up" id="up_<?php echo $row[0] ?>"></span>
					<span class="rate" id="rate_<?php echo $row[0] ?>"><?php echo $row[11] ?></span> <span class="rate-down" id="down_<?php echo $row[0] ?>"></span>
				</div>
...
<div class='hiddenComm'>" .	stripslashes(check_words(trim($context))) . "</div>";
				...
				?>
				<span class="reply"><a title="<?php echo _REPLY ?>" href="javascript:void(0);" onclick="mtReplyCommentOnClick(<?php echo $row[0] ?>, '<?php echo $row[5] ?>')"><?php echo _REPLY ?></a>
				</span>
				<span class="cdelete" style="display:none"><a title="<?php echo _DELETE ?>" href="javascript:void(0);" onclick="javascript:cdeleteOnClick(<?php echo $row[0] ?>, '<?php echo $row[5] ?>')"><?php echo _DELETE ?></a>
				</span>
		...
		?>
			...
			<div id="commentform">
				<div class="<?php echo langStyle('align') ?>">
					<form method="post" action="modules.php?name=<?php echo $module_name ?>&amp;file=comments" name="comments_form" id="comments-form" onsubmit="return mtCommentOnSubmit()">
						<?php
						...
						?>
						<input required aria-required="true" autocomplete="on" id="author-name" name="author" type="text" value="<?php echo (empty($userinfo['username']) ? _NAME : $userinfo['username']) ?>" <?php echo (empty($userinfo['username']) ? "" : "readonly") ?> onfocus="if (this.value == '<?php echo _NAME; ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo _NAME; ?>';}" onfocus="mtCommentFormOnFocus()" />
						<input required aria-required="true" autocomplete="on" id="author-email" name="email" type="email" value="<?php echo (empty($userinfo['user_email']) ? _EMAIL : $userinfo['user_email']) ?>" onfocus="if (this.value == '<?php echo _EMAIL ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo _EMAIL ?>';}" onfocus="mtCommentFormOnFocus()" /> <br />
						<input id="author-url" name="url" type="text" value="<?php echo (empty($userinfo['user_website']) ? 'http://" onfocus="mtCommentFormOnFocus()' : $userinfo['user_website']) ?>" onfocus="if (this.value == '<?php echo 'http://'; ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo 'http://'; ?>';}" onfocus="mtCommentFormOnFocus()" />
						<br style="clear:both">
						...
						<div class="clear"></div>
						<button type="submit" class="submit" accesskey="s" name="post" id="comment_submit" onclick="mtCommentOnSubmit()"><?php echo _SEND ?></button>
						<div id="comment-form-reply" style="color:red;font-weight:700;display: none"><input type="checkbox" id="comment-reply" name="comment_reply" value="" onclick="mtSetCommentParentID()" /><label class="comment-cookie" for="comment-reply" id="comment-reply-label"></label>
...
<?php
	...
	{
		global $module_name, $user, $admin, $user_prefix, $prefix, $anonpost, $articlecomm, $db, $notify, $notify_subject, $notify_email, $notify_from;
		...
		if ($fake == 1 and $articlecomm == 1) {
			...
			if (is_admin($admin)) {
				$db->sql_query("INSERT INTO " . COMMENTS_TABLE . " (tid,pid,sid,active,date,name,email,url,host_name,subject,comment,score,reason) VALUES
				 (NULL, '$comment_reply', '$sid', '1' , now(), '$author', '$email', '$url', '$ip', '$title_story', '$comment', '0', '0')") or die('');
				...
				if ($notify == "1") {
					$message = "<b>$author</b> By $email " . _COMMENTS_SNEDERS . " :<br><br>" . _URL . ":  " . USV_DOMAIN . $Story_address . " <br><br> " . _LASTIP . " : $ip <br><br>" . _COMMENT . ":    $comment  <br><br><hr><br> مدیریت : <br> تایید این نظر : <br> " . USV_DOMAIN . "admin.php?op=moderation_mc_view&id=" . $db->sql_nextid() . "";
