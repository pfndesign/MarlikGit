...
var simpleTree;
var structureManagerURL = "modules.php?app=mod&name=treemenu";
...
function TreeOperations()
{
	...
	{    // Menu de yeni eleman ekle se�enegi tiklandiginda ilk bura �agrilir
		...
		function(evt)
		{
			if (evt.keyCode == 13) // when pressed enter
			{
				...
				var params = encodeURI("action=insertElement"+"&tname="+name+"&ownerEl="+ownerEl+"&slave="+slave);
	...
	this.updateElementNameReq = function()
	{
		...
		function(evt)
		{
			if (evt.keyCode == 13) { //pressed enter
				var name = $('#inputText').attr('value');
				var ownerEl = $('#inputText').parent().parent().parent().attr('id');
				var params = "action=updateElementName&tname="+name+"&elementId="+elementId+"&ownerEl="+ownerEl;
	...
	* Options From
	* ******************************/
	this.optionsForm = function (){
		var elementId = treeOps.trGetSelected().attr('id');
		var params = 'action=getelementOpts&elementId='+elementId;
		$.get(structureManagerURL+"&"+params,function (data){
			var lmnt = new Array();
			lmnt = explode("$sep$",data);
			var ilink = new Array();
			ilink = explode("|",lmnt[2]);
			$("#eid").val(elementId);
			$("#titlein").val(lmnt[0]);
			$("#lang").val(lmnt[1]);
			$("#link").val(ilink[0]);
			$("#ltarget").val(ilink[1]);
			$("#modulename").val(lmnt[3]);
			$("#icon").val(lmnt[4]);
			var icon = $("#icon").val();
		$("#icon_prev").html('<img src="images/icon/'+icon+'" />');
			$(".menufrmdiv").fadeIn('slow');
		});
