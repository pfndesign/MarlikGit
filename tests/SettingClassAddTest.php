...
final class SettingClassAddTest extends TestCase
{
    /**
     * @dataProvider AddSettingProvider 
     * @dataProvider BulkAddSettingProvider
     */
    public function testBulkAddLocalSetting($key, $value, $expected, bool $serialize): void
    {
        ...
        foreach ($key as $keyindex => $keyname) {
            ...
            $this->assertEqualsWithDelta(is_array($expected) && isset($expected[$keyname]) ? $expected[$keyname] : $expected, $settinginstance->get($keyname, null, false), 0.1);
            ...
            $this->assertEqualsWithDelta(is_array($expected) && isset($expected[$keyname]) ? $expected[$keyname] : $expected, $reinitsetting->get($keyname, null, false), 0.1);
            //remove it
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testBulkAddDatabaseSetting($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testAddSettingWithModifier($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
        ...
        foreach ($key as $keyindex => $keyname) {
            //updated instance check
            $this->assertEqualsWithDelta(is_array($value) && isset($value[$keyname]) && array_values($key) == array_keys($value) ? $value[$keyname] : $value, $settinginstance->get($keyname, null, true, "sampledemodifer"), 0.1);
            $this->assertEqualsWithDelta("modified:" . samplemodifer((is_array($expected) && isset($expected[$keyname]) ? $expected[$keyname] : $expected), false, $serialize), $settinginstance->get($keyname, null, false), 0.1);
            $this->assertEqualsWithDelta(is_array($value) && isset($value[$keyname]) && array_values($key) == array_keys($value) ? $value[$keyname] : $value, $reinitsetting->get($keyname, null, true, "sampledemodifer"), 0.1);
            $this->assertEqualsWithDelta("modified:" . samplemodifer((is_array($expected) && isset($expected[$keyname]) ? $expected[$keyname] : $expected), false, $serialize), $reinitsetting->get($keyname, null, false), 0.1);
            //remove it
            $settinginstance->remove($keyname);
            $reinitsetting->remove($keyname);
        }
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testAddSettingWithModifierInDatabase($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testAddSettingLocalWithModifierClass($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
        ...
        foreach ($key as $keyindex => $keyname) {
            //updated instance check
            $this->assertEqualsWithDelta(is_array($value) && isset($value[$keyname]) && array_values($key) == array_keys($value) ? $value[$keyname] : $value, $settinginstance->get($keyname, null, true, [$this, "sampledemodiferclass"]), 0.1);
            $this->assertEqualsWithDelta("modified:" . samplemodifer((is_array($expected) && isset($expected[$keyname]) ? $expected[$keyname] : $expected), false, $serialize), $settinginstance->get($keyname, null, false), 0.1);
            $this->assertEqualsWithDelta(is_array($value) && isset($value[$keyname]) && array_values($key) == array_keys($value) ? $value[$keyname] : $value, $reinitsetting->get($keyname, null, true, [$this, "sampledemodiferclass"]), 0.1);
            $this->assertEqualsWithDelta("modified:" . samplemodifer((is_array($expected) && isset($expected[$keyname]) ? $expected[$keyname] : $expected), false, $serialize), $reinitsetting->get($keyname, null, false), 0.1);
            //remove it
            $settinginstance->remove($keyname);
            $reinitsetting->remove($keyname);
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testAddSettingDatabaseWithModifierClass($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
