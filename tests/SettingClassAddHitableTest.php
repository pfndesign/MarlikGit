...
final class SettingClassAddHitableTest extends TestCase
{
    /**
     * @dataProvider AddSettingProvider 
     * @dataProvider BulkAddSettingProvider
     */
    public function testAddHitableSetting($key, $value, $expected, bool $serialize): void
    {
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testAddHitableSettingLocal($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
        $settinghitconfig = [
            "minupdaterate" => "1minute",
        ...
        addSetting($key, $value, null, "local", $serialize, $settinghitconfig);
        //reinit class check
        $reinitsetting = new Setting();
        if (!is_array($key))
            $key = [0 => $key];
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testupdateHitableSetting($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
        $settinghitconfig = [
            "minupdaterate" => "1minute",
        ...
        addSetting($key, $value, null, "database", $serialize, $settinghitconfig);
        $settinghitconfig2 = [
            "minupdaterate" => "2minute",
            "collectfunction" => false,
            "deletehitsaftercollect" => false,
        ];
        addSetting($key, $value, null, "database", $serialize, $settinghitconfig2);
        //reinit class check
        $reinitsetting = new Setting();
        if (!is_array($key))
            $key = [0 => $key];
        foreach ($key as $keyindex => $keyname) {
            ...
            $config = $settinginstance->get_hitable_config($keyname);
            unset($config['nextupdate']);
            $this->assertEquals($config, $settinghitconfig2);
            $this->assertEqualsWithDelta(is_array($value) && isset($value[$keyname]) && array_values($key) == array_keys($value) ? $value[$keyname] : $value, $reinitsetting->get($keyname, null, true, [$this, "sampledemodiferclass"]), 0.1);
            $this->assertEqualsWithDelta("STORE:DB", $reinitsetting->get($keyname, null, false), 0.1);
            $this->assertTrue($reinitsetting->is_hitable($keyname));
            $config = $reinitsetting->get_hitable_config($keyname);
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testConvertNotHitableToHitableSetting($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
     ...
     * @dataProvider BulkAddSettingProvider
     */
    public function testConvertHitableToNotHitableSetting($key, $value, $expected, bool $serialize): void
    {
        global $settinginstance;
     ...
     */
    public function testIsHitableFunctionValidFunction($value, $expected): void
    {
        global $settinginstance;

        $settinghitconfig = [
            "minupdaterate" => "2minute",
            "collectfunction" => $value,
            "deletehitsaftercollect" => false,
        ];
        addSetting("testdbhitable", $value, null, "database", false, $settinghitconfig);
        $config = $settinginstance->get_hitable_config("testdbhitable");
        $collectfunction = $settinginstance::is_function_valid($config['collectfunction']);
        $this->assertSame($expected, $collectfunction);
        //reinit class check
        $reinitsetting = new Setting();
        $config = $reinitsetting->get_hitable_config("testdbhitable");
        $collectfunction = $reinitsetting::is_function_valid($config['collectfunction']);
        $this->assertSame($expected, $collectfunction);

        $settinginstance->remove("testdbhitable");
        $reinitsetting->remove("testdbhitable");
    }
