...
switch ($op) {
    ...
    case "BackupDB":
        ...
        function get_table_def($dbuname, $table, $crlf)
        {
            $schema_create = "";
            global $drop, $db;
            if ($drop == 1) {
            ...
            mysqli_select_db($db->db_connection, $dbuname);
            $result = $db->sql_query("SHOW FIELDS FROM $table") or mysql_die();
            ...
            mysqli_select_db($db->db_connection, $dbuname);
            $result = $db->sql_query("SHOW KEYS FROM $table") or mysql_die();
            while ($row = mysqli_fetch_array($result)) {
                $kname = $row['Key_name'];
        ...
        mysqli_select_db($db->db_connection, $dbname) or die("Unable to select database");
        ...
        if (is_array($tablelist) && count($tablelist) > 0) {
            ...
            $num_tables = $db->sql_numrows($tablelist);
    ...
    case "StatusDB":
        ...
        if ($num_tables > 0) {
            ...
            for ($j = 0; $j < $numfields; $j++) {
                echo "<td align=\"center\"><b>" . $db->sql_fieldname($j) . "</b></td>";
            }
            echo "</tr>";
            while ($row = mysqli_fetch_row($result)) {
