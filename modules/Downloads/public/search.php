...
$query = sql_quote($_POST['query']);
if (empty($query)) {
	show_error("ورودی در بخش جستجو دریافت فایل وارد نکرده اید . بازگردید و مجدد جستجو کنید");
}
...
if ($query != "") {
  ...
  if ($crows > 0) {
    ...
    while($cidinfo2 = $db->sql_fetchrow($result2)) {
      ...
      $cidinfo3['title'] = str_replace($query, "<b>$query</b>", $cidinfo3['title']);
