...
require_once("modules/Web_Links/l_config.php");
 ...
 if (isset($url) && (!preg_match('#^http[s]?\\:\\/\\/[a-z0-9\-]+\.([a-z0-9\-]+\.)?[a-z]+#i', $url))) {
  	     die("Illegal url...");
  	 }
...
function menu($mainlink) {
	...
	."<font class=\"content\"><input type=\"text\" size=\"25\" name=\"query\" onkeypress=\"return farsikey(this,event)\" onkeydown=changelang(this);> <input type=\"submit\" value=\""._SEARCH."\"><br><IMG src=\"images/fa.gif\" align=\"absbottom\" style=\"CURSOR: hand\" onclick=change(query)></font>"
	."</form>";
...
function SearchForm() {
	...
	."<tr><td><font class=\"content\"><input type=\"text\" size=\"25\" name=\"query\" onkeypress=\"return farsikey(this,event)\" onkeydown=changelang(this);> <input type=\"submit\" value=\""._SEARCH."\"><br><IMG src=\"images/fa.gif\" align=\"absbottom\" style=\"CURSOR: hand\" onclick=change(query)></td></tr>"
	."</table>"
	."</form>";
}
...
function index() {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		$cdescription = stripslashes(check_html($row['cdescription'], ""));
...
function AddLink() {
	...
	.""._YOURNAME.": <input type=\"text\" name=\"auth_name\" size=\"30\" maxlength=\"60\" onkeypress=\"return farsikey(this,event)\" onkeydown=changelang(this);> <IMG src=\"images/fa2.gif\" align=\"absbottom\" style=\"CURSOR: hand\" onclick=change(auth_name)><br>"
	.""._YOUREMAIL.": <input type=\"text\" name=\"email\" dir=\"ltr\" size=\"30\" maxlength=\"60\"><br><br>"
...
function Add($title, $url, $auth_name, $cat, $description, $email) {
	...
	if ($numrows>0) {
		...
		$url = check_words(check_html($url, "nohtml"));
		$description = check_words(check_html($description, ""));
		$auth_name = check_words(check_html($auth_name, "nohtml"));
		if (!empty($email)) { 
...
function NewLinks($newlinkshowdays) {
	...
	while ($counter <= $newlinkshowdays-1) {
		$newlinkdayRaw = (time()-(86400 * $counter));
		$newlinkday = date("d-M-Y", $newlinkdayRaw);
        $newlinkView = date("Y-m-d", $newlinkdayRaw);
        $newlinkView = hejridate($newlinkView, 1, 4);
        $newlinkDB = Date("Y-m-d", $newlinkdayRaw);
		$totallinks = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_links_links WHERE date LIKE '%$newlinkDB%'"));
		$counter++;
		$allweeklinks = $allweeklinks + $totallinks;
...
function NewLinksDate($selectdate) {
    ...
    $dateView = (date("Y-m-d", $selectdate));
    $dateView = hejridate($dateView, 1, 4);
	include("header.php");
	menu(1);
	echo "<br>";
	OpenTable();
	...
	while ($row2 = $db->sql_fetchrow($result2)) {
		...
		$description = stripslashes(check_html($row2['description'], ""));
		$time = $row2['date'];
...
function TopRated($ratenum, $ratetype) {
	...
	if ($toplinkspercentrigger == 1) {
		...
		echo "<center><font class=\"option\"><b>"._BESTRATED." ".htmlspecialchars($toplinks)." </b></font></center><br>";
	}
	echo "</td></tr>"
...
function MostPopular($ratenum, $ratetype) {
	...
	if ($mostpoplinkspercentrigger == 1) {
		...
		echo "<center><font class=\"option\"><b>"._MOSTPOPULAR." ".htmlspecialchars($mostpoplinks)."</b></font></center>";
	}
	...
	while($row3 = $db->sql_fetchrow($result3)) {
		...
		$description = stripslashes(check_html($row3['description'], ""));
		$time = $row3['date'];
...
function RandomLink() {
	...
	if (!empty($row2)) {
		...
		Header("Location: modules.php?name=Web_Links");
	}
...
function viewlink($cid, $min, $orderby, $show) {
	...
	while($row2 = $db->sql_fetchrow($result2)) {
		...
		$cdescription2 = stripslashes(check_html($row2['cdescription'], ""));
	...
	while($row4 = $db->sql_fetchrow($result4)) {
		...
		$description = stripslashes(check_html($row4['description'], ""));
		$time = $row4['date'];
...
function search($query, $min, $orderby, $show) {
	...
	$x=0;
    $the_query = check_words(check_html(addslashes(FixQuotes($query)), "nohtml"));
	menu(1);
	echo "<br>";
	OpenTable();
	if ($query != "") {
		if ($nrows>0) {
			...
			while ($row2 = $db->sql_fetchrow($result2)) {
				...
				$title3 = preg_replace("/$query/i", "<b>$query</b>", $title3);
			...
			while($row = $db->sql_fetchrow($result)) {
				...
				$linkratingsummary = number_format($linkratingsummary, $mainvotedecimal);
				$transfertitle = str_replace (" ", "_", $title);
				$title = preg_replace("/$query/i", "<b>$query</b>", $title);
				echo "<a href=\"modules.php?name=$module_name&amp;l_op=visit&amp;lid=$lid\" target=\"new\">$title</a>";
				newlinkgraphic($datetime, $time);
				popgraphic($hits);
				echo "<br>";
				$description = preg_replace("/$query/i", "<b>$query</b>", $description);
				echo ""._DESCRIPTION.": $description<br>";
				setlocale (LC_TIME, $locale);
			    if (preg_match ("/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})/", $time, $datetime)) {
                $datetime = hejridate($time, 4, 4);
...
function viewlinkeditorial($lid) {
	...
	echo "<br>";
	OpenTable();
	//echo "<center><font class=\"option\"><b>"._LINKPROFILE.": ".htmlspecialchars($displaytitle)."</b></font><br>";
	linkinfomenu($lid, $ttitle);
	if ($recordexist != 0) {
		while($row = $db->sql_fetchrow($result)) {
			$adminid = intval($row['adminid']);
			$editorialtimestamp = $row['editorialtimestamp'];
			$editorialtext = check_html($row['editorialtext'], "");
			$editorialtitle = check_html($row['editorialtitle'], "nohtml");
			if (preg_match ("/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})/", $editorialtimestamp, $editorialtime)) {
            $editorialtime = hejridate($editorialtimestamp, 4, 4);
            }
            $formatted_date = $editorialtime;
			echo "<br><br>";
			OpenTable2();
...
function viewlinkcomments($lid) {
	...
	$displaytitle = $transfertitle;
	OpenTable();
	//echo "<center><font class=\"option\"><b>"._LINKPROFILE.": ".htmlspecialchars($displaytitle)."</b></font><br><br>";
	linkinfomenu($lid, $ttitle);
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		if (preg_match ("/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})/", $ratingtimestamp, $ratingtime)) {
                $ratingtime = hejridate($ratingtimestamp, 4, 4);
        }
        $formatted_date = $ratingtime;
...
function outsidelinksetup($lid) {
    ...
    <span align=\"left\" dir=\"ltr\">"._PROMOTE02."</span><br><br>
    ...
    <center align='left' dir='ltr'><i>&lt;a href=\"$nukeurl/modules.php?name=$module_name&amp;l_op=ratelink&amp;lid=$lid\"&gt;"._RATETHISSITE."&lt;/a&gt;</i></center>
    <br><br>
    ...
    <table border=\"0\" align=\"center\"><tr><td align=\"left\" dir=\"ltr\"><i>
    ...
    <br>"._HTMLCODE3."<br><br></center><span align=\"left\" dir=\"ltr\">
...
function brokenlinkS($lid,$cid, $title, $url, $description, $modifysubmitter) {
	...
	if (is_user($user)) {
		...
		$title = check_words(check_html(addslashes($title), "nohtml"));
  	    ...
  	    $modifysubmitter = check_words(check_html(addslashes($modifysubmitter), "nohtml"));
...
function modifylinkrequest($lid) {
	...
	if ($blocknow != 1) {
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			.""._URL.":<br><input type=\"text\" name=\"url\" dir=\"ltr\" value=\"$url\" size=\"50\" maxlength=\"100\"><br><br>"
...
function addrating($ratinglid, $ratinguser, $rating, $ratinghost_name, $ratingcomments) {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		/* Passed Tests */
		if ($passtest == "yes") {
			 $comment = check_html(ratingcomments, "");
			 if (!empty($comment)) {
				update_points(16);
			}
			update_points(15);
...
function ratelink($lid, $user) {
	global $prefix, $db,$cookie, $datetime, $module_name, $user_prefix;
	include("header.php");
	menu(1);
	echo "<br>";
	OpenTable();
	...
	$transfertitle = preg_replace("/_/", " ", $ttitle);
	$displaytitle = $transfertitle;
	$displaytitle = check_html($row['title'], "nohtml");
	if(isset($_SERVER['REMOTE_HOST'])) { $ip = $_SERVER['REMOTE_HOST'];}
	if (empty($ip)) {
		$ip = $_SERVER['REMOTE_ADDR'];
	}
	echo "<b>".htmlspecialchars($displaytitle)."</b>"
