...
if (($radminsuper==1) OR ($radminuser==1)) {
	...
	$result1 = $db->sql_query("SELECT * FROM ".$user_prefix."_cnbya_value_temp WHERE uid='$chng_uid' AND fid='$t'");
		while ($tmpsqlvalue = $db->sql_fetchrow($result1)) {
			...
			echo "<tr><td bgcolor='$bgcolor2'>$name_exit</td><td><b><input type='text' value='$tmp_value' size='40' disabled=disabled style='color=#000000;background-color: #FFFFFF'>></b>";
			echo "</td></tr>\n";
		}
    }
    ...
    echo "<tr><td colspan=\"2\" align=\"left\"><br>\n";
        ...
        echo "<td width=\"3\"></td>\n";
			if ($ya_config['useactivate'] == 0) {
				echo "<form action='".ADMIN_OP."mod_users' method='post'><td valign=\"top\">\n";
				if (isset($min)) { echo "<input type='hidden' name='min' value='$min'>\n"; }
