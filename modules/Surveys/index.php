...
if(!isset($pollID)) {
	...
	$result = $db->sql_query("SELECT pollID, pollTitle, voters FROM ".$prefix."_poll_desc where artid='0' AND active='1' order by timeStamp DESC limit 1,5");
	while ($row = $db->sql_fetchrow($result)) {
		$plid = intval($row['pollID']);
		$pltitle = stripslashes(check_words(check_html($row['pollTitle'], "nohtml")));
...
function pollMain($pollID) {
	...
	$result_a = $db->sql_query("SELECT pollTitle, voters FROM ".$prefix."_poll_desc WHERE pollID='$pollID'   AND active='1'");
...
function pollLatest() {
	...
	if ($multilingual == 1) {
		$querylang = "WHERE planguage='$currentlang' AND artid='0'   AND active='1'";
	} else {
		$querylang = "WHERE artid='0'   AND active='1'";
	}
		...
		$row3 = $db->sql_fetchrow($result3);
			...
			if(!empty($optionText)) {
				...
				$percent2 = (int)$percent;
					...
					$ThemeSel = get_theme();
					if (file_exists("themes/$ThemeSel/images/mainbar.gif")) {
						define("SURVEY_IMG_PATH","themes/$ThemeSel/images/");
					}else {
						define("SURVEY_IMG_PATH","modules/$module_name/images/");
					}
					...
					$l_size = getimagesize(SURVEY_IMG_PATH.$leftbar);
					$m_size = getimagesize(SURVEY_IMG_PATH.$mainbar);
					$r_size = getimagesize(SURVEY_IMG_PATH.$rightbar);
					
					
				if (file_exists("themes/$ThemeSel/images/mainbar.gif")) {
					$m1_size = getimagesize("themes/$ThemeSel/images/mainbar.gif");
					$mainbar_d = "survey_mainbar_d.gif";
					if ($percent2 > 0 AND $percent2 <= 23) {
						$salto = "<img src=\"".SURVEY_IMG_PATH."$mainbar\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m_size[1]\" width=\"$percentInt\">";
					} elseif ($percent2 > 24 AND $percent2 < 50) {
						$a = $percentInt - 100;
						$salto = "<img src=\"".SURVEY_IMG_PATH."$mainbar\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m_size[1]\" width=\"70\">"
						."<img src=\"".SURVEY_IMG_PATH."$mainbar_d\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m1_size[1]\" width=\"30\">"
						."<img src=\"".SURVEY_IMG_PATH."$mainbar\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m_size[1]\" width=\"$a\">";
					} elseif ($percent2 > 49 AND $percent2 < 75) {
						$a = $percentInt - 200;
						$salto = "<img src=\"".SURVEY_IMG_PATH."$mainbar\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m_size[1]\" width=\"70\">"
						."<img src=\"".SURVEY_IMG_PATH."$mainbar_d\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m1_size[1]\" width=\"30\">"
						."<img src=\"".SURVEY_IMG_PATH."$mainbar\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m_size[1]\" width=\"70\">"
						."<img src=\"".SURVEY_IMG_PATH."$mainbar_d\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m1_size[1]\" width=\"30\">"
						."<img src=\"".SURVEY_IMG_PATH."$mainbar\" alt=\"$percent2 %\" title=\"$percent2 %\" height=\"$m_size[1]\" width=\"$a\">";
				...
				$percentInt = $percentInt -100;
				
				if ($percent > 0) {
					echo "<img src=\"".SURVEY_IMG_PATH."$leftbar\" height=\"$l_size[1]\" width=\"$l_size[0]\" alt=\"$percent2 %\" title=\"$percent2 %\">";
				    echo "<img src=\"".SURVEY_IMG_PATH."$mainbar\" height=\"$m_size[1]\" width=\"$percentInt\" alt=\"$percent2 %\" title=\"$percent2 %\">";
					echo "<img src=\"".SURVEY_IMG_PATH."$rightbar\" height=\"$r_size[1]\" width=\"$r_size[0]\" alt=\"$percent2 %\" title=\"$percent2 %\">";
				} else {
					echo "<img src=\"".SURVEY_IMG_PATH."$leftbar\" height=\"$l_size[1]\" width=\"$l_size[0]\" alt=\"$percent2 %\" title=\"$percent2 %\">";
					if (!file_exists("themes/$ThemeSel/images/survey_mainbar_d.gif")) {
						echo "<img src=\"".SURVEY_IMG_PATH."$mainbar\" height=\"$m_size[1]\" width=\"$m_size[0]\" alt=\"$percent2 %\" title=\"$percent2 %\">";
					}
					echo "<img src=\"".SURVEY_IMG_PATH."$rightbar\" height=\"$r_size[1]\" width=\"$r_size[0]\" alt=\"$percent2 %\" title=\"$percent2 %\">";
				}
