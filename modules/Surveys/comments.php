...
function navbar($pollID, $title, $thold, $mode, $order) {
	...
	$title = check_html($row['pollTitle'], "nohtml");
                  cookiedecode($user);
                  getusrinfo($user);
                  if (!isset($mode) OR empty($mode)) {
                    if(isset($userinfo['umode'])) {
   ...
   $comments = 0;
	...
	if ($mode == 'nested') {
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			$r_date = $row['date'];
            $r_name = check_html($row['name'], "nohtml");
			$r_email = check_html($row['email'], "nohtml");
			$r_host_name = check_html($row['host_name'], "nohtml");
			$r_subject = check_html($row['subject'], "nohtml");
			$r_comment = check_html($row['comment'], "");
			...
			if($r_score >= $thold) {
				...
				$url = check_html($row_url['user_website'], "nohtml");
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				$url = check_html($row_url2['user_website'], "nohtml");
...
function DisplayTopic ($pollID, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
	...
	while ($count_times < $num_tid) {
		...
		$subject = check_words(check_html($row_q['subject'], "nohtml"));
		$comment = check_words(check_html($row_q['comment'], ""));
		...
		// note: FB you dirty bastard :D - Quake - (serious you are)
		...
		if(is_admin($admin)) {
			...
			$host_name = check_html($row3['host_name'], "nohtml");
...
function reply ($pid, $pollID, $mode, $order, $thold) {
	...
	if ($anonpost == 0 AND !is_user($user)) {
		...
		."<TEXTAREA wrap=virtual cols=70 rows=15 name=comment onkeypress=\"return farsikey(this,event)\" onkeydown=changelang(this);></TEXTAREA><br><IMG src=\"images/fa.gif\" align=\"absbottom\" style=\"CURSOR: hand\" onclick=change(comment)><br>
...
function replyPreview ($pid, $pollID, $subject, $comment, $xanonpost, $mode, $order, $thold) {
	...
	."<TEXTAREA wrap=\"virtual\" cols=\"70\" rows=\"15\" name=\"comment\" onkeypress=\"return farsikey(this,event)\" onkeydown=changelang(this);>$comment</TEXTAREA><br><IMG src=\"images/fa.gif\" align=\"absbottom\" style=\"CURSOR: hand\" onclick=change(comment)><br>";
...
// Quake - end
    $pollID = intval($pollID);
    $pid = intval($pid);
	$author = check_words(check_html(addslashes($author), "nohtml"));
	$subject = check_words(check_html(addslashes($subject), "nohtml"));
	$comment = format_url($comment);
	$comment = check_words(check_html(addslashes($comment), ""));
